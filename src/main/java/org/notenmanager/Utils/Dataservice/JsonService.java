...
public class JsonService implements DataService {
    ...
    public JsonService() {
        users = GetFromFile(getUserListPath());
    ...
    private static <T> List<T> GetFromFile(String fileLocation) {
        List<T> list = new ArrayList();
        ObjectMapper mapper = new ObjectMapper();
        mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        try {
            list = mapper.readValue(new File(fileLocation), new TypeReference<List<T>>() {
            });
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public List<SchoolSubject> GetSchoolSubjectsFromUser(User user) {
        subjects= GetFromFile(getSchoolSubjectListPath(user.username));
        return subjects;
    ...
    public void UpdateSchoolSubjectForUser(User user, String nameOfSubjectToUpdate, SchoolSubject schoolSubject) {
        for(SchoolSubject subject:subjects){
            ...
            if(subject.name==nameOfSubjectToUpdate){
